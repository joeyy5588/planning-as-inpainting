...
from eval.conditional_pipeline import Conditional2DPipeline
from utils.helpers import apply_conditioning_2d

logger = get_logger(__name__, log_level="INFO")

def train_conditional_2d(config, model, text_encoder, tokenizer, noise_scheduler, optimizer, train_dataloader, val_dataloader, evaluator, lr_scheduler, ema_model=None):
    ...
    for epoch in range(config.num_epochs):
        ...
        if accelerator.is_main_process:
            if epoch % config.eval_per_epoch == 0 or epoch == config.num_epochs - 1:
                ...
                pipeline = Conditional2DPipeline(unet=unet, text_encoder=text_encoder, tokenizer=tokenizer, scheduler=noise_scheduler)
