...
class GridRepresentationDataset(torch.utils.data.Dataset):
...
def generate_new_path(grid_size=10, max_objects=5, min_object=3, max_obstacles=20, min_obstacles=10):
    ...
    observations = np.concatenate([repr_grid, np.expand_dims(path, axis=0)], axis=0)
    actions = []
    terminals = []
